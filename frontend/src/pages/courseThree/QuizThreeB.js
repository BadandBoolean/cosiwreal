...
export default function QuizTwoB({ makeUpdateScore, setCourseTwoColour }) {
    ...
    return (<>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', width: '100%', margin: '20px' }}>
            ...
            <div className={styles.quizWrapper}>
                ...
                <div className={styles.questionsection}>
                    <p>Question 6: What is an example of something that can be returned by the server to the client, besides an HTTP response?</p>
                    <form className={styles.questions} onSubmit={handleSubmit}>
                        <label>
                            <input type='radio' value="JSON data" checked={quizAnswer == "JSON data"} onChange={handleChange} />
                            <span> JSON data</span>
                        </label>
                        <label>
                            <input type='radio' value="A Microsoft Word Document" checked={quizAnswer == "A Microsoft Word Document"} onChange={handleChange} />
                            <span> A Microsoft Word Document</span>
                        </label>
                        <label>
                            <input type='radio' value="A fax" checked={quizAnswer == "A fax"} onChange={handleChange} />
                            <span> A fax</span>
                        </label>
                        <label>
                            <input type='radio' value="A redirect to a different webpage" checked={quizAnswer == "A redirect to a different webpage"} onChange={handleChange} />
        ...
        {(answerResult == "correct") && <div style={{width: '100%', display: 'flex', justifyContent: 'center', margin: '10px', padding: '0px'}}><div style={{cursor: 'pointer', position: 'relative'}}><p onClick={updateProgress} className={styles.underlineanim}><Link href='/' legacyBehavior><p style={{textDecoration: 'none'}}>Congratulations! You have completed Course Three. Click here to go back to the Homepage</p></Link></p></div></div>}
    </>);
}