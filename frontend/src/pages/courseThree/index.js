        ...
        <>
            <div>
                ...
                <div style={{display: 'flex', justifyContent: 'center', width: '80%', flexDirection: 'column'}}>
                    {!startClicked && <motion.div variants={textVar} initial="before" animate="notclicked" style={{display: 'flex', justifyContent: 'center', flexBasis: '100%', margin: '30px', fontSize: '25px'}}>
                    Drag to open a connection with the server and send a request
                    </motion.div>
                    }
                    <div style={{display: 'flex', justifyContent: 'center'}}>
                    <AnimatePresence> {!startClicked  && 
                        (<motion.div 
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
                            whileHover={{scale: 1.1}}
                            exit={{ opacity: 0, transition: {opacity:{duration: 1, type: 'ease'}} }}
                            key='1' 
                            ref={constraintsRef} 
                            style={{display: 'flex', flexBasis: '50%', height: '60px', borderRadius: '20px', background: '#ec058e'}}>
                         <motion.div
                         drag="x"
                         ...
                         initial={{ opacity: 0 }}
                         animate={{ opacity: 1 }}
                         exit={{ opacity: 0, scale: 0, transition: {scale: {duration: 1, type: 'ease'}, opacity:{duration: 1, type: 'ease'}} }}
                         style={{borderRadius: '20px', background: 'white', margin: '2px', width: '65px', cursor: 'pointer'}}
                         onDragEnd={
                             ()=>toggleStartClicked(true)
                         }
                         >&nbsp;</motion.div>
                        </motion.div>)
                    }</AnimatePresence></div>
                    <AnimatePresence>{!!codeShowing && (
                        <Listening key={1} toggleProcessInfo={toggleProcessInfo} />
                    )}</AnimatePresence>
                    <AnimatePresence>
                        {!!processInfo && (
                            <PiecesProcessed key={3} />
                        )}
                    </AnimatePresence>
                </div>     
                </div>
